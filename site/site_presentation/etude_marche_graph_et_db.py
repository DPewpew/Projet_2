...
# Config Streamlit
# -----------------------------
st.set_page_config(page_title="Projet 2 — Graphs (v1)", layout="wide")
...
# -----------------------------
st.sidebar.markdown("### 1️⃣ Étude de marché")
...
st.sidebar.markdown("---")
st.sidebar.markdown("### 2️⃣ Traitement de la DB")
show_db = st.sidebar.button("Ouvrir la section DB")

st.sidebar.markdown("---")
st.sidebar.markdown("### 3️⃣ Machine Learning")
show_ml = st.sidebar.button("Ouvrir la section ML")

st.sidebar.markdown("---")
st.sidebar.markdown("### 4️⃣ Tout afficher")
show_all = st.sidebar.button("Afficher toute l'étude de marché")
...
def graph_5(df_sal_age: pd.DataFrame):
    st.subheader("Graph 5 — Salaire net mensuel moyen par catégorie socioprofessionnelle (2023)")
...
# ---- SECTION DB ----
    ...
    st.header("2️⃣ Traitement de la base de données")
    st.info("Espace réservé au nettoyage, fusion, enrichissement des bases (IMDB / CNC / API).")
    st.text_area("Code / notes DB", height=400)

# ---- SECTION ML ----
elif show_ml:
    st.header("3️⃣ Machine Learning")
    st.info("Espace réservé au système de recommandation.")
    st.text_area("Code / notes ML", height=400)

# ---- ÉTUDE DE MARCHÉ (PAR DÉFAUT) ----
else:
